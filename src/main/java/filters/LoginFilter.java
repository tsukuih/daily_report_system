...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        //HttpServletRequestからコンテキストパスを取得し、String型のcontextPath変数に代入
        String contextPath = ((HttpServletRequest) request).getContextPath();

        //HttpServletRequestからサーブレットパスを取得し、String型のservletPass変数に代入
        String servletPass = ((HttpServletRequest) request).getServletPath();

        if(servletPass.matches("/css.*")) {
            // cssフォルダ内は認証処理から除外する
            ...
            String action = request.getParameter(ForwardConst.ACT.getValue()); //action
            String command = request.getParameter(ForwardConst.CMD.getValue()); //command
            ...
            if (ev == null) {
                ...
                if(!(ForwardConst.ACT_AUTH.getValue().equals(action)
                    ...
                    //リダイレクト先：/daily_report_system/?action=Auth&command=showLogin
                ...
                if(ForwardConst.ACT_AUTH.getValue().equals(action)) {
                    ...
                    if(ForwardConst.CMD_LOGIN.getValue().equals(command)) {
                        ...
                        // 上記以外の認証系Actionはエラーの画面

                        String forward = String.format("/WEB-INF/views/%s.isp", "error/unknown");
